...
class KITTIPCDClsDataset(Dataset):
    def __init__(self, root_dir, filename_labels, split='train'):
        ...
        self.label2ind = {label: ind for ind, label in enumerate(self.labels)}
        self.ind2label = {ind: label for ind, label in enumerate(self.labels)}

        self.__N = 64 # num. of obeservations in point cloud
        
        
        with open(os.path.join(self.data_root, filename)) as f:
    ...
    def __getitem__(self, idx):
        ...
        filename_pc = os.path.join(self.data_root, label, pc_ind+'.txt')
...
class KITTIPCDClsDataset_Wrapper(object):
    ...
    def get_dataloader(self):
        # load training set
        train_dataset = KITTIPCDClsDataset(self.data_root, self.filename_labels, 'train')
        ...
        train_loader = DataLoader(dataset=train_dataset, shuffle=False, batch_size=self.bs, sampler=train_sampler, num_workers=self.num_workers, drop_last=True)
        valid_loader = DataLoader(dataset=train_dataset, shuffle=False, batch_size=self.bs, sampler=val_sampler, num_workers=self.num_workers, drop_last=True)

        # load test set
        test_dataset = KITTIPCDClsDataset(self.data_root, 'object_names.txt', 'test')
