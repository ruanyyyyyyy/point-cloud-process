# The implementation of kdtree, including building and searching
...
def sort_key_by_vale(key, value):
    """
    Sort values before building trees. The order of keys should change too.
    Input: 
        key, value
    Output:
...
def kdtree_recursive_build(root, db, point_indices, axis, leaf_size):
    ...
    Input:
        root: tree's root
        db: point clouds data
        axis: scalar
        leaf_size: scalar
    Output:
        root: a built tree's root
    """
    if root is None:
        root = Node(axis, None, None, None, point_indices) # (self, axis, value, left, right, point_indices)
    ...
    if len(point_indices) > leaf_size:
        ...
        root.value = (middle_right_point_value + middle_left_point_value) * 0.5
...
def kdtree_knn_search(root: Node, db: np.ndarray, result_set: KNNResultSet, query: np.ndarray):
    """
    knn search through kdtree. Find the nearest k neighbors.
    Input:
        root: kd tree
        db: original data
        result_set: searching results
        query: indexing info
    Output:
...
def kdtree_radius_search(root: Node, db: np.ndarray, result_set: RadiusNNResultSet, query: np.ndarray):
    """
    Radius searching through kdtree. Find the nearest neighbors in radius range.
    Input:
        root: kdtree
        db: original data
        result_set: searching results
