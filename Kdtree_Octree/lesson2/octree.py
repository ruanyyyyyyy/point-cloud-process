# The implementation of octree, including building and searching
...
def octree_recursive_build(root, db, center, extent, point_indices, leaf_size, min_extent):
    ...
    Input:
        ...
        db: original data
        center: the center
        extent: current segment interval
        point_indices: the key of points
        leaf_size: scale
        min_extent: the minimum segment interval
    """
    ...
    if len(point_indices) <= leaf_size or extent <= min_extent:
        ...
        factor = [-0.5, 0.5]
        for i in range(8):
            # determine child center and extent
...
def overlaps(query: np.ndarray, radius: float, octant:Octant):
    ...
    if int(np.sum(query_offset_abs < octant.extent)) >= 2:
        return True
...
def octree_radius_search(root: Octant, db: np.ndarray, result_set: RadiusNNResultSet, query: np.ndarray):
    """
    Search neighbors in radius in octree
    Input:
        root
        db
        result_set
...
def octree_knn_search(root: Octant, db: np.ndarray, result_set: KNNResultSet, query: np.ndarray):
    """
    Find k nearest neighor in octree
    Input:
        root
        db
        result_set
...
def octree_construction(db_np, leaf_size, min_extent):
    """
    Build octree. Call octree_recursive_build interface
    Input:
        dp_np
        leaf_size
        min_extent
